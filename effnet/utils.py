...
def load_pretrained_weights(model, model_name, weights_path=None, load_fc=True, advprop=False):
    ...
    if load_fc:
        ...
        head_layers = ['_conv_head.weight', '_bn1.weight', '_bn1.bias', '_bn1.running_mean', 
                       '_bn1.running_var', '_bn1.num_batches_tracked', '_fc.bias', '_fc.weight']
        for layer in head_layers:
            state_dict.pop(layer)
        ret = model.load_state_dict(state_dict, strict=False)
        #assert set(ret.missing_keys) == set(head_layers), f'Missing keys when loading pretrained weights: {ret.missing_keys}'
